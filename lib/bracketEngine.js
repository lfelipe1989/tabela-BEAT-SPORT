...
export function advanceDoubleElim(state, includeThirdPlace) {
  ...
  // cruzamos pra formar as duas semifinais (cada invicta pega um
  // sobrevivente, nunca duas invictas entre si nem dois sobreviventes).
  if (wbSurvivors && lbSurvivors && !semifinals) {
    semifinals = [
      { id: uid('sf'), teamA: wbSurvivors[0], teamB: lbSurvivors[1], sets: [], winner: null },
      { id: uid('sf'), teamA: wbSurvivors[1], teamB: lbSurvivors[0], sets: [], winner: null },
    ];
  }
